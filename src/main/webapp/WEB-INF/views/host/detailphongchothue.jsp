		...
		<div class="iright">
			...
			<form action="<c:url value="/become-a-host/detailplace/add-image/${detailPlace.placeId }" />" method="post" enctype="multipart/form-data">	
				<div class="imgcha">
					<c:forEach var="item" items="${detailPlace.image }">
						<div class="imgcon">
							<img alt="" width="200px" height="135px" src="<c:url value="/asset/images/${item }" />">
							<a href="<c:url value="/become-a-host/detailplace/delete-image/${detailPlace.placeId }?image=${item }" />">
			...
			<br>
				...
				<%-- <form action="<c:url value="/become-a-host/detailplace/editdetail/${detailPlace.detailId.detailId }" />" method="post"> --%>
					...
					<input type="number" value="${detailPlace.detailId.phongkhach }" name="khach" id="khach" class="khach" /> <br> 
					...
					<input type="number" value="${detailPlace.detailId.phongngu }" name="phongngu" id="phongngu" class="khach" /> 
					<label>Giường: </label> 
					<input type="number" value="${detailPlace.detailId.giuong }" name="giuong" id="giuong" class="khach" />
					<label>Phòng vệ sinh: </label> 
					<input type="number" value="${detailPlace.detailId.phongvs }" name="phongvs" id="phongvs" class="khach" />  <br> 
					<input value="${detailPlace.detailId.detailId }" name="detailId" id="detailId"  hidden />  <br> 
		...
		<script type="text/javascript">
			function editDetailPlace(){
				...
				$.ajax({
					type : "GET",
					contentType : "application/json",
					url : "/AirTrip/become-a-host/detailplace/editdetail",
					data : {
						detailId : detailId,
						khach: khach,
						phongngu: phongngu,
				...
				$.ajax({
					...
					success : function(data) {
						...
						var result = '<div id="reac-avt-'+data.reactionId +'" class="reac-avt-'+data.reviewId+'">'
								+ ' 	<div class="avt">'
								+ ' 		<img alt="" width="50px" height="50px" src="<c:url value="/asset/images/'+data.account.image +'" />">'
								+ ' 	</div>'
								+ ' 	<div >'
								+ ' 		<b>'+userName+'</b>'
								+ ' 		<p>'+data.dateSubmit +'</p>'
    ...
    <script type="text/javascript">
			...
			function editTitle(){
				...
				$.ajax({
					type : "POST",
					contentType : "application/x-www-form-urlencoded;charset=utf-8",
					url : "/AirTrip/become-a-host/detailplace/editplace",
					dataType: "json",
					data : jQuery.param({
						placeId : placeId,
						tieude: tieude,
